...
class FeedScreenTest {
    ...
    /**
     * Критерии приёмки 5 и 6. Известную причину лента называет словами вместо общего «в очереди», и
     * сегодня таких слов ровно две пары: «нет ключа ElevenLabs» и «ошибка ElevenLabs <код>» — без
     * них отозванный ключ и ключ с опечаткой выглядят для владельца одинаково, и заметка тихо не
     * появляется. Обрыв сети сюда не попадает намеренно: воркер причину в этом случае снимает, и
     * остаётся общее «в очереди — расшифровка». Начало тела ответа сюда не едет — на него места
     * нет, оно живёт в плашке деталки.
     */
    @Test
