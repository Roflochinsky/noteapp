...
class RepoStore(
    ...
    fun refresh(force: Boolean = false): SyncStatus {
        ...
        return try {
            val was = snapshot
            // Ручное обновление спрашивает безусловно: поведение ETag на `git/ref` за CDN не
            // проверено (research §10.2), а владелец, дёрнувший экран вниз, обязан получить
            // свежее состояние, а не «у нас записано, что не менялось» (граница среза Н7).
