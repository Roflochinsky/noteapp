...
Кладутся рядом с экраном в `app/src/test/kotlin/.../ui/`, образцы — `TasksScreenTest.kt` и
`TaskDetailScreenTest.kt`. Классу **обязателен `@GraphicsMode(GraphicsMode.Mode.NATIVE)`**: без
него у Robolectric нет шрифта и текст меряется как 1px на символ (`#дом` — 4px вместо 33px), а на
таких метриках любая проверка геометрии — тач-таргет, попадание, перенос — либо врёт, либо
проходит сама собой. Проверять поведение, а не реализацию: семантику (клик, роль, описание) и
размер узла, а не то, каким модификатором он получен.
...
ставит прогоны в очередь через `flock` и — главное — ходит `--offline`.

**Почему `--offline` (замер 2026-08-29, цифры настоящие):** конфигурация сборки шла
**280 секунд**, из них 279 — молчаливое ожидание таймаутов сети при разрешении
зависимостей. Это тот же обрыв до внешних хостов из WSL, что ловили с GitHub. С
`--offline` конфигурация — **1 секунда**, полный гейт с APK — **19 секунд вместо пяти
минут**. Все зависимости лежат в `~/.gradle/caches`. Сменил версию зависимости — тогда
`bin/gate --online`, и жди.

Второй ограничитель, уже не такой злой: память. Машина 16 ядер / 11 ГБ, один прогон — это
Gradle-демон (2 ГБ) плюс Kotlin-демон (1,5 ГБ), двое собирающих разом уводят систему в
своп. Поэтому `bin/gate` и держит очередь на один прогон; ждущий прогон убивать не надо.
Агент, который НЕ собирает, стоит ~0,5 ГБ — таких можно держать сколько угодно.

Остальное из того же замера:

- **`--rerun-tasks` в заданиях агентам не писать** (`bin/gate` его и не пропустит): он тянет
  полную перекомпиляцию, по 5 минут на КАЖДУЮ задачу вместо 19 секунд на весь гейт.
- Кэш сборки общий на все worktree (`org.gradle.caching=true`) — без него каждое дерево
  агента компилирует одно и то же заново.
- Демоны после снятых агентов остаются жить и держат по гигабайту: `./gradlew --status`,
  прибить те, что в `IDLE`.
